...
import net.katsstuff.arbaro.mesh.MeshSection;
...
public class TreePreview extends JComponent {
	...
	public void paint(Graphics g) {
		...
		try {
			...
			initTransform(/*g*/);
			if (draft) {
				previewTree.traverseTree(new StemDrawer(g)); //drawStems(g);
			} else {
				drawMesh(g);
				//drawLeaves(g);
...
/*			Enumeration e = previewTree.allStems(showLevel-1);
			...
			*/
			...
			if (aStem != null) {
				...
				// get greatest distance from orig
				x = Math.max(
					Math.abs(min.getX() - orig.getX()),
					Math.abs(max.getX() - orig.getX())
				);
				y = Math.max(
					Math.abs(min.getY() - orig.getY()),
