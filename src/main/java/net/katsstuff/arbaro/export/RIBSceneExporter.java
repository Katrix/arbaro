...
/**
 * Creates a RenderMan scene file with the rendered tree included.
 */
class RIBSceneExporter extends AbstractExporter {
	...
	public void doWrite() {
		...
		println("Option \"searchpath\" \"shader\" [ \""
				+ windowsPathToUNC(System.getProperty("user.dir"))
				+ "/renderman:@\" ]");
		...
		println("ScreenWindow "
				+ frm.format(-halfWidth)
				+ " "
				+ frm.format(halfWidth)
				+ " "
				+ frm.format(-halfHeight)
				+ " "
				+ frm.format(halfHeight));
		...
		println();
		println(
			"LightSource \"shadowdistant\" \"key\" \"from\" [ 5000 5000 -3000 ] \"intensity\" [ 1.2 ] \"string shadowname\" [ \"raytrace\" ] \"float samples\" [ 1 ]");
		println(
			"LightSource \"pointlight\" \"fill\" \"from\" [ -5000 2000 3000 ] \"intensity\" [ 0.5 ] \"float decay\" [ 0 ]");
		println();
