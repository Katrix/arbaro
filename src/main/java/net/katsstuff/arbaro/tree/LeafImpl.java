...
//		par = params;
	...
	private void setLeafOrientation(Params par) {
		...
		// rotation up
		norm = transf.getY();
		double fbend = Vector.atan2(
			Math.sqrt(norm.getX() * norm.getX() + norm.getY() * norm.getY()),
			norm.getZ()
		);
...
//			.rotaxis(orientation,Vector.Z_AXIS);
	...
	/**
	 * Makes the leave. Does nothing at the moment, because all the values can be calculated in the constructor
	 */
	...
	/* (non-Javadoc)
	 * @see net.katsstuff.arbaro.tree.TraversableLeaf#traverseTree(net.katsstuff.arbaro.tree.TreeTraversal)
	 */
