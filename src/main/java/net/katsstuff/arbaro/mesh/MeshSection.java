 ...
 * <p>
 * This is a number of vertices in one layer. Several layers build the mesh part for a stem.
 */
...
//		segment = seg; 
	...
	public int faceCount(boolean useQuads) {
		if (size() == 1) {
			return next.size();
		} else if (next.size() == 1 || useQuads) {
			return size();
		} else {
			return size() * 2; // two triangles per vertice (quad)
		}
	}
	...
	public Vector normalAt(int i) throws Exception {
		...
		if (v.normal == null) {
			throw new MeshException("Error: Normal not set for point "
									+ vectorStr(v.point));
		}
		return v.normal;
	}
